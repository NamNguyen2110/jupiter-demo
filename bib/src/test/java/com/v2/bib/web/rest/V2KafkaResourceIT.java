...
import com.v2.bib.config.EmbeddedKafka;
import com.v2.bib.config.KafkaSseConsumer;
import com.v2.bib.config.KafkaSseProducer;
...
class V2KafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/v-2-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/v-2-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/v-2-kafka/unregister"));
                return;
            }
        }
